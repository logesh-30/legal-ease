...
const data = JSON.parse(
  readFileSync(new URL("./india_government_schemes_2026_enriched.json", import.meta.url), "utf-8")
);
...
async function seed() {
  ...
  const docs = data.schemes.map((s: any) => ({
    ...
    // Map documents_required array → flat string arrays
    documentsEn:   Array.isArray(s.documents_required)
      ? s.documents_required.map((d: any) => `${d.document} — ${d.purpose}`)
      : s.documents_en ? [s.documents_en] : [],
    documentsTa:   s.documents_ta ? [s.documents_ta] : [],
  }));
