...
router.post("/save/:type/:id", requireAuth, async (req: AuthedRequest, res) => {
  ...
  if (req.params.type === "service") {
    ...
    if (!existing.includes(saveId)) user.savedServices.push(saveId as any);
  } else {
    const existing = user.savedSchemes.map((v) => v.toString());
    if (!existing.includes(saveId)) user.savedSchemes.push(saveId as any);
  }
  await user.save();
  return res.json({ message: "Saved" });
});
router.get("/saved", requireAuth, async (req: AuthedRequest, res) => {
